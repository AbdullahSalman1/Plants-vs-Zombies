...
class Sunflower_fields :public level {
private:
	...
	Sprite peeshooterSprite; // Declare Sprite objects here
...
public:
	...
	GameState repeaterState = Idle;
	GameState snowState = Idle;
	Sunflower_fields(RenderWindow& window) {
		drawmap(window, "Fullpatch.jpg");


		if (!sunlightTexture.loadFromFile("suno.png")) {
			cout << "Failed to load sunlight image in sunflower fields_level!" << endl;
			return;
		}
		...
		if (!sunflowerTexture.loadFromFile("sunflowerCard.png")) {
			cout << "Failed to load sunflower selection image in sunflower fields_level!" << endl;
			return;
		}
		...
		// Placement of repeater card
		if (!repeaterTexture.loadFromFile("repeaterCard.png")) {
			cout << "Failed to load repeater selection image!" << endl;
			return;
		}
		repeaterSprite.setTexture(repeaterTexture);
		repeaterSprite.setPosition(0, 340); // Adjust position as needed
		
		// Placement of snow card
		if (!snowTexture.loadFromFile("snowcard.png")) {
			cout << "Failed to load snow selection image!" << endl;
			return;
		}
		snowSprite.setTexture(snowTexture);
		snowSprite.setPosition(0, 425); // Adjust position as needed 
	...
	void run(Event event, RenderWindow& window, Score& score, level* level_1) {
		...
		if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left) {
			...
			//repeater selection condition
			if (mouseX >= repeaterSprite.getPosition().x && mouseX <= repeaterSprite.getPosition().x + repeaterSprite.getLocalBounds().width &&
				mouseY >= repeaterSprite.getPosition().y && mouseY <= repeaterSprite.getPosition().y + repeaterSprite.getLocalBounds().height) {
				if (score.getScore() >= 200) {
					level_1->plants_factory_getter()->add_plant('r');
					repeaterState = repeaterposition;
					score.subtractScore(200);
				}
			}
			//snow selection condition
			if (mouseX >= snowSprite.getPosition().x && mouseX <= snowSprite.getPosition().x + snowSprite.getLocalBounds().width &&
				mouseY >= snowSprite.getPosition().y && mouseY <= snowSprite.getPosition().y + snowSprite.getLocalBounds().height) {
				if (score.getScore() >= 100) {
					level_1->plants_factory_getter()->add_plant('x');
					snowState = snowPosition;
					score.subtractScore(100);
				}
			}
			else if (peeshooterState == peeshooterPosition) {
				if (mouseX >= 300 && mouseX <= 1200 && mouseY >= 90 && mouseY <= 680) {
					mouseX -= 300;
					int count = 0;
					while (mouseX >= 100) {
						mouseX -= 100;
				...
				if (mouseX >= 300 && mouseX <= 1200 && mouseY >= 90 and mouseY <= 680) {
					mouseX -= 300;
					int count = 0;
					while (mouseX >= 100) {
						mouseX -= 100;
			...
			else if (snowState == snowPosition) {
				if (mouseX >= 300 && mouseX <= 1200 && mouseY >= 90 && mouseY <= 680) {
					mouseX -= 300;
					int count = 0;
					while (mouseX >= 100) {
			...
			else if (repeaterState == repeaterposition) {
				if (mouseX >= 300 && mouseX < 1200 && mouseY >= 90 and mouseY <= 680) {
					mouseX -= 300;
					int count = 0;
					while (mouseX >= 100) {
