...
using namespace std;
#include"plant.h"
class peeshooter : public plants {
...
public:
    peeshooter() : plants("peashooter.png") {
        numBullets = 0;
        if (!fireTexture.loadFromFile("pea.png")) {
            cout << "Failed to load peeshooter pea image!" << endl;
        }
    ...
    void fire_update(int i, plants* currentPlant)override {
        ...
        if (fireTimer.getElapsedTime().asSeconds() >= 3) { // Adjust firing rate as needed
            // Create a new bullet
            if (numBullets < 10) {
