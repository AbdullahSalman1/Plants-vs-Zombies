...
public:
    snowpee() : plants("snow.png") {
        numBullets = 0;
        if (!fireTexture.loadFromFile("spea.png")) {
            cout << "Failed to load snow pee bullet image!" << endl;
        }
    ...
    void fire_update(int i, plants* currentPlant)override {
        ...
        if (fireTimer.getElapsedTime().asSeconds() >= 6) {
            // Create a new bullet
            if (numBullets < 10) {
                ...
                //   cout << "numbullets " << numBullets << endl;
                fireTimer.restart();
            }
        }
